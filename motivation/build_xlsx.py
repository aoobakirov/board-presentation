...
Три уровня суммы (на руки / брутто-оклад / полная стоимость) + ячейка-переключатель базы деления.
Листы: «Матрица 75-25», «Показатели (детально)», «Справочник показателей». Только русские термины."""
...
HDR=Font(name='Calibri',bold=True,color=WHITE,size=9)
...
# №, ФИО, подразделение, должность, тип, на_руки, брутто(оклад), полная_стоимость, примечание, [(показатель,вес,прибыль?)]
EMP=[
 (8,'Серикбаев Адиль','Руководство','Президент','front',5000000,6000139,6613107,'Отвечает за весь результат',
 ...
 (4,'Данияр Алим','Руководство','Генеральный директор','front',1000000,1246243,1422801,'Операционный результат',
 ...
 (2,'Аубакиров Алимбек','Финансы','Финансовый директор','front',3500000,4347889,4861722,'Прибыль и денежная устойчивость',
 ...
 (11,'Унбетпаев Ернар','Операции','Операционный директор','front',1500000,1873611,2124280,'Валовая прибыль по операциям ≥ K× стоимости',
 ...
 (5,'Доолоткулов Данияр','Логистика','Начальник отдела логистики','front',1000000,1246243,1422801,'Валовая прибыль отдела ≥ K× стоимости',
 ...
 (13,'Сибагатов Хамардин','Логистика','Менеджер по логистике (старший)','front',800000,993718,1140528,'Маржа ≥ K× стоимости; наставничество',
 ...
 (6,'Жунусова Камила','Логистика','Менеджер по логистике','front',200000,230896,261836,'Маржа ≥ K× стоимости',
   [('Валовая маржа по своим перевозкам (≥K×стоимости)',0.45,1),('Объём перевозок к норме',0.15,0),
    ('Соблюдение сроков доставки',0.15,0),('Загрузка / порожний пробег',0.15,0),('Точность документов / сохранность',0.10,0)]),
 (7,'Сейтқадыр Дарын','Логистика','Менеджер по логистике','front',200000,236143,274587,'Маржа ≥ K× стоимости',
   [('Валовая маржа по своим перевозкам (≥K×стоимости)',0.45,1),('Объём перевозок к норме',0.15,0),
    ('Соблюдение сроков доставки',0.15,0),('Загрузка / порожний пробег',0.15,0),('Точность документов / сохранность',0.10,0)]),
 (12,'Белова Полина','Логистика','Менеджер по логистике (младший)','front',150000,173011,201177,'Маржа ≥ K× стоимости; пороги ниже',
   [('Валовая маржа по своим перевозкам (≥K×стоимости)',0.45,1),('Объём перевозок к норме',0.15,0),
    ('Соблюдение сроков доставки',0.15,0),('Загрузка / порожний пробег',0.15,0),('Точность документов / сохранность',0.10,0)]),
 (9,'Таджимурадова Асем','Расчёты','Менеджер по закрытию','front',300000,378788,440456,'Прибыль по закрытым перевозкам + собранная оплата',
 ...
 (1,'Аманбекова Айгуль','Персонал','Руководитель отдела персонала','back',1000000,1246243,1379182,'Поддержка · доля от прибыли компании',
 ...
 (3,'Байльденова Шолпан','Персонал','Менеджер по персоналу','back',500000,614931,715042,'Поддержка · доля от прибыли компании',
 ...
 (10,'Ташметов Сакен','Юридический','Юрист','back',1000000,1246243,1422801,'Поддержка · доля от прибыли компании',
...
subtotal_ref={}
r=4
for (idx,name,dep,role,typ,net,gross,full,note,kpis) in EMP:
...
# =========== ЛИСТ: Матрица 75-25 (три уровня + переключатель базы) ===========
mx=wb.create_sheet('Матрица 75-25',0); mx.sheet_view.showGridLines=False
mx['A1']='ТОО «WayStar Group» · Матрица оплаты 75/25 и показателей'
mx['A1'].font=Font(name='Calibri',bold=True,size=14,color=NAVY)
mx['A2']='Приложение 1 к приказу (75/25, с 01.01.2026). Три уровня суммы: «на руки», «брутто (оклад)», «полная стоимость (ФОТ)». '\
         'База деления 75/25 выбирается ячейкой-переключателем ниже; премия = Лимит × мин(100%; факт), не выше лимита (приказ).'
mx['A2'].font=Font(name='Calibri',size=9,italic=True,color='6B7A8D'); mx.merge_cells('A2:N2')
# переключатель базы
mx['A3']='База деления 75/25:'; mx['A3'].font=BOLD; mx.merge_cells('A3:B3')
sw=mx['C3']; sw.value='Брутто (оклад)'; sw.font=Font(bold=True,color='1F4E79',size=11); sw.fill=fill(YELLOW)
sw.alignment=CTR; sw.border=border; mx.merge_cells('C3:D3')
dvs=DataValidation(type='list',formula1='"На руки,Брутто (оклад),Полная стоимость (ФОТ)"',allow_blank=False)
mx.add_data_validation(dvs); dvs.add(sw)
mx['E3']='← выберите: на руки / брутто (оклад) / полная стоимость (ФОТ)'; mx['E3'].font=Font(size=9,italic=True,color='6B7A8D')
mx.merge_cells('E3:N3')

heads=['Подразделение','Должность','ФИО','Таб. №','На руки (100%), ₸','Брутто/оклад (100%), ₸','Полная стоимость (100%), ₸',
       'Целевой доход по базе (100%), ₸','Оклад (75%), ₸','Лимит премии (25%), ₸','Дата начала','Факт KPI, %',
       'Начисленная премия, ₸','Итого (оклад+премия), ₸']
hr=5
for j,txt in enumerate(heads,1):
    c=mx.cell(hr,j,txt); c.font=HDR; c.fill=fill(NAVY); c.border=border; c.alignment=CTR
widths=[13,24,21,7,13,14,14,15,13,13,10,9,14,15]
...
for (idx,name,dep,role,typ,net,gross,full,note,kpis) in EMP:
    ...
    nc=mx.cell(row,5,net); nc.number_format='#,##0'; nc.font=REG; nc.alignment=RIGHT
    gc=mx.cell(row,6,gross); gc.number_format='#,##0'; gc.font=REG; gc.alignment=RIGHT
    fc=mx.cell(row,7,full); fc.number_format='#,##0'; fc.font=REG; fc.alignment=RIGHT
    # целевой доход по выбранной базе
    tc=mx.cell(row,8,'=IF($C$3="На руки",E%d,IF($C$3="Брутто (оклад)",F%d,G%d))'%(row,row,row))
    tc.number_format='#,##0'; tc.font=Font(name='Calibri',bold=True,size=10,color=STEEL); tc.alignment=RIGHT
    ok=mx.cell(row,9,'=H%d*%s'%(row,FIX)); ok.number_format='#,##0'; ok.font=REG; ok.alignment=RIGHT
    lim=mx.cell(row,10,'=H%d*%s'%(row,VAR)); lim.number_format='#,##0'; lim.font=REG; lim.alignment=RIGHT
    mx.cell(row,11,DATE).font=Font(size=9); mx.cell(row,11).alignment=CTR
    fk=mx.cell(row,12,"='Показатели (детально)'!E%d"%subtotal_ref[idx]); fk.number_format='0%'
    fk.font=Font(name='Calibri',size=10,color=GREEN,bold=True); fk.alignment=CTR
    pr=mx.cell(row,13,'=J%d*MIN(1,L%d)'%(row,row)); pr.number_format='#,##0'; pr.font=BOLD; pr.alignment=RIGHT
    it=mx.cell(row,14,'=I%d+M%d'%(row,row)); it.number_format='#,##0'; it.font=BOLD; it.alignment=RIGHT
    zb=LIGHT if (row-first_data)%2==0 else WHITE
    for j in range(1,15):
        mx.cell(row,j).border=border
        if j!=12: mx.cell(row,j).fill=fill(zb)
    row+=1
last_data=row-1
mx.cell(row,3,'ИТОГО').font=Font(bold=True,color=WHITE,size=10)
for col in (5,6,7,8,9,10,13,14):
...
for j in range(1,15):
    ...
    mx.cell(row,j).alignment=RIGHT if j in (5,6,7,8,9,10,13,14) else CTR
trow=row
row+=2
mx.cell(row,3,'В год (×12), ₸:').font=BOLD
mx.cell(row,8,'=H%d*12'%trow).number_format='#,##0'; mx.cell(row,8).font=Font(bold=True,color=NAVY); mx.cell(row,8).alignment=RIGHT
mx.cell(row,10,'=J%d*12'%trow).number_format='#,##0'; mx.cell(row,10).font=Font(bold=True,color=COPPER); mx.cell(row,10).alignment=RIGHT
mx.cell(row,13,'=M%d*12'%trow).number_format='#,##0'; mx.cell(row,13).font=Font(bold=True,color=COPPER); mx.cell(row,13).alignment=RIGHT
mx.cell(row,14,'=N%d*12'%trow).number_format='#,##0'; mx.cell(row,14).font=Font(bold=True,color=GREEN); mx.cell(row,14).alignment=RIGHT
row+=2
for note in [
 'Три уровня: «на руки» (нетто), «брутто/оклад» (начисленный), «полная стоимость» (ФОТ со всеми налогами работодателя).',
 'Ячейка C3 — переключатель базы: «Целевой доход по базе» и деление 75/25 пересчитываются автоматически.',
 'Для трудового договора оклад указывается в брутто. При базе «на руки»/«полная стоимость» оклад (75%) — пересчитать в брутто перед договором.',
 'Начисленная премия = Лимит × мин(100%; Факт KPI). По приказу (п.4.3) премия не превышает лимит (25%).',
 'Факт KPI, % — с листа «Показатели (детально)» (вводится там). Таб. № — временный, заменить на реальные ID.']:
    mx.cell(row,1,'• '+note).font=Font(size=9,italic=True,color='6B7A8D')
    mx.merge_cells(start_row=row,start_column=1,end_row=row,end_column=14); row+=1
mx.freeze_panes='A6'

# =========== ЛИСТ: Справочник показателей ===========
sp=wb.create_sheet('Справочник показателей'); sp.sheet_view.showGridLines=False
...
sw2=[16,42,20,14,20,10]
for j,w in enumerate(sw2,1): sp.column_dimensions[get_column_letter(j)].width=w
rr=5
